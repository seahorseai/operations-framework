...
resource "aws_db_instance" "rds_mysql" {
  ...
  tags = {
    Name = "rds-mysql-lambda-backup"
    Environment = "DEV"
    Application = "rds-mysql-lambda-backup"
  }
}

/*
resource "aws_db_instance" "rds-mysql-lambda-backup" {
  identifier = "rds-mysql-database-${local.env}"
  instance_class = "db.t2.micro"
  engine = "MySQL"
...
/*resource "aws_security_group" "saints-xctf-database-security" {
  ...
  tags = {
    Name = "saints-xctf-database-security-${local.env}"
    Environment = upper(local.env)
    Application = "lambda-mysql-rds-backup"
  }
}*/