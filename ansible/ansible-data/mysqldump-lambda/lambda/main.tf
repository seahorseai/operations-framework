...
data "archive_file" "rds-backup-lambda" {
...
data "aws_iam_role" "rds-access-role" {
  name = "rds-backup-lambda-role"
}

data "aws_db_instance" "rds_mysql" {
  db_instance_identifier = "rds-mysql-${local.env}"
}

resource "aws_lambda_function" "rds-backup-lambda" {
  function_name = "rds-backup${upper(local.env)}"
  ...
  role = data.aws_iam_role.rds-access-role.arn
  runtime = "python3.7"
  timeout = 15
  ...
  environment {
    variables = {
      ENV = local.env
      DB_HOST = data.aws_db_instance.rds_mysql.address
    }
  }
  ...
  tags = {
    ...
    Environment = "DEV"
    Application = "rds-backup-lambda"
  }
}


resource "aws_lambda_alias" "rds-backup-lambda" {
  name = "rds-backup-lambda"
  description = "AWS Lambda function which creates MySQL database backups"
  function_name = aws_lambda_function.rds-backup-lambda.function_name
  function_version = "$LATEST"
}



/*resource "aws_lambda_permission" "rds-backup-lambda" {
